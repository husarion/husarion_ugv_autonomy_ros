...
bool DockRobot::setGoal(Goal & goal)
{
  if (!this->getInput<std::string>("dock_type", goal.dock_type) || goal.dock_type.empty()) {
    RCLCPP_ERROR_STREAM(
      this->logger(), husarion_ugv_manager::GetLoggerPrefix(name()) << "Failed to get input [dock_type]");
    return false;
  }
  ...
  if (
    ...
    goal.use_dock_id) {
    RCLCPP_ERROR_STREAM(this->logger(), husarion_ugv_manager::GetLoggerPrefix(name()) << "Failed to get input [dock_id]");
    return false;
  }

  if (!this->getInput<bool>("navigate_to_staging_pose", goal.navigate_to_staging_pose)) {
    RCLCPP_WARN_STREAM(
      this->logger(), husarion_ugv_manager::GetLoggerPrefix(name()) << "navigate_to_staging_pose not set, using default "
                                                 "value: "
  ...
  if (
    ...
    goal.navigate_to_staging_pose) {
    RCLCPP_ERROR_STREAM(
      this->logger(), husarion_ugv_manager::GetLoggerPrefix(name()) << "Failed to get input [max_staging_time]");
    return false;
  }
...
{
  RCLCPP_ERROR_STREAM(
    this->logger(), husarion_ugv_manager::GetLoggerPrefix(name()) << ": onFailure with error: " << toStr(error));
  return BT::NodeStatus::FAILURE;
}
